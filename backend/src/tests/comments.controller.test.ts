...
describe('CommentsController', () => {
    ...
    test('should fetch comments for a given post id (default pagination)', async () => {
        const mockComments = { items: [], total: 0, page: 1, totalPages: 0, message: "No comments found for post 1" };
        mockCommentService.getCommentsByPostId.mockResolvedValue(mockComments);
        mockReq.params = { postId: '1' };

        await controller.fetchCommentsByPost(mockReq as Request, mockRes as Response);

        expect(mockCommentService.getCommentsByPostId).toHaveBeenCalledWith(1, 1, 10, expect.any(Object), expect.any(Object));
    ...
    test('should accept page & size query params and return correct slice of comments', async () => {
        ...
        expect(mockCommentService.getCommentsByPostId).toHaveBeenCalledWith(1, 2, 15, expect.any(Object), expect.any(Object));
    });
    ...
    test('should include total comment count and totalPages metadata', async () => {
        ...
        expect(mockRes.json).toHaveBeenCalledWith({items: [], total: 0, page: 1, totalPages: 0, message: "No comments found for post 1"});
    });
    ...
    test('each comment should include author profile resolved from PocketBase without sensitive fields', async () => {
        ...
        expect(response.items[0].author).toEqual(expect.objectContaining({
    ...
    test('should support filtering comments by authorId and by date range', async () => {
        ...
        expect(new Date(response.items[0].created_at).getUTCMilliseconds()).toBeLessThanOrEqual(new Date(endDate).getUTCMilliseconds());
        expect(new Date(response.items[0].created_at).getUTCMilliseconds()).toBeGreaterThanOrEqual(new Date(startDate).getUTCMilliseconds());
    });

    test('should support searching comment text with a query parameter', () => {expect(true).toBe(false)});

    test('should paginate nested replies when include=replies is requested', () => {expect(true).toBe(false)});

    test('should include reply_count and a preview of recent replies for each comment', () => {expect(true).toBe(false)});

    test('should not expose internal database fields (e.g., internal ids, soft-delete flags) in response', () => {expect(true).toBe(false)});

    test('should respect comment visibility/soft-delete (hide deleted or moderated comments)', () => {expect(true).toBe(false)});

    test('should return 401 when authentication is required and token is missing or invalid', () => {expect(true).toBe(false)});

    test('should return 403 when authenticated user lacks permission to view restricted comments', () => {expect(true).toBe(false)});

    test('should sanitize comment content (strip unsafe HTML / sanitize markdown) before returning', () => {expect(true).toBe(false)});

    test('should include attachments/meta for comments when requested and handle missing attachments gracefully', () => {expect(true).toBe(false)});

    test('should stream or handle large comment payloads without blocking (resilience)', () => {expect(true).toBe(false)});

    test('should enforce a maximum page size and reject overly large page size requests', () => {expect(true).toBe(false)});

    test('should return 500 on unexpected database or service (PocketBase) errors', () => {expect(true).toBe(false)});

    test('should map commenter profile fallbacks when PocketBase user is missing', () => {expect(true).toBe(false)});

    test('should allow clients to request only specific fields to reduce payload (fields=...)', () => {expect(true).toBe(false)});
