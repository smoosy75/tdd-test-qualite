...
import { FilterOptions, ICommentService, SortOptions } from "../services/commentService";
...
export class CommentsController {
  ...
  async fetchCommentsByPost(req: Request, res: Response) {
    ...
    const limit = typeof req.query.size === "string" ? parseInt(req.query.size, 10) : 10;
    if (isNaN(limit) || limit < 1) {
        return res.status(400).json({ error: "Invalid size parameter. Must be a positive integer" });
    }
    ...
    const sort = typeof req.query.sort === "string" ? req.query.sort.toLowerCase() : "desc";
    ...
    if (!allowedSortFields.includes(sortField)) {
      return res.status(400).json({ 
        error: "Invalid sort field. Allowed fields: createdAt, updatedAt, likes" 
      });
    }

    try 
    {
      const commentsData = await this.commentService.getCommentsByPostId(postId, page, limit, { sort, sortField } as SortOptions, filters);
      ...
      const userIds = [...new Set(commentsData.items.map(comment => comment.user_id))];
